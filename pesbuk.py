...
def menu(my_name,my_id):
	...
	print('🥀→5. Hasil Crack  ')
	print('🥀→0. Keluar       ')
	_____fazry__sablenk_____ = input('\n>> Pilih : ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Om ? : '))
	except ValueError:
...
def grup():
	...
	if berr2=='Masuk Facebook':
		...
		fazry_sablenk('>> Grup Tidak Di Temukan ')
		time.sleep(0.5)
		grup()
	else:pass
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/FACEBOOK-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/FACEBOOK-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/FACEBOOK-DUMP/'+geh,'r').read().splitlines()
		except:
...
#			id2.insert(xx,bacot)
		...
		exit()
	print('>> 1. Mobile (\x1b[1;92mDisarankan\33[m) ')
	print('>> 2. Mbasic ')
	print('>> 3. Touch  ')
	print('>> 4. Mtouch ')
	print('')
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/FACEBOOK-DUMP')
	except:pass
	try:os.system('touch .prox.txt')
	except:pass
	try:os.system('pkg install neofetch -y')
	except:pass
	try:os.system('clear')
	except:pass
	login()
